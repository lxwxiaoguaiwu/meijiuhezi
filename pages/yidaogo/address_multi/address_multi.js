...
Page({
  ...
  savePersonInfo: function(e) {
    ...
    if (data.name == '') {
      ...
      wx.request({
        url: 'https://api.changching.cn/api/v3/alcoaddresses',
        method: 'POST',
        data: {
          userid: userInfo.id,
          tel: data.tel,
          addressinfo: data.province + data.city + data.district + data.address,
          toname: data.name
        },
        header: {
          'Accept': "*/*",
      ...
      wx.navigateTo({
        url: '/pages/yidaogo/place/place?toname=' + data.name + '&tel=' + data.tel + '&addressinfo=' + data.province + data.city + data.district + data.address + '&leixing=address' + '&id=' + this.data.id + '&name=' + this.data.wname + '&num=' + this.data.num + '&pic=' + this.data.pic+'&price='+this.data.price,
      })
    }
  },
